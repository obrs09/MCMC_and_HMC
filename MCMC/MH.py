...
# def binorm(x, y, mu_x, mu_y, sigma_x, sigma_y, rho):
#     result = 1/(2*np.pi*sigma_x*sigma_y*np.sqrt(1-rho*rho))*np.exp(-1/(2*(1-rho*rho))*(((x-mu_x)/sigma_x)**2-2*rho*((x-mu_x)/sigma_x)*((y-mu_y)/sigma_y)+((y-mu_y)/sigma_y)**2))
#     return result

class MH(object):
    def __init__(self, y_1, y_2, sigma_x, sigma_y, rho, MAX, theta_1_0, theta_2_0, scale):
        self.y_1 = y_1
        self.y_2 = y_2
    ...
    def MH_al(self, y_1, y_2, sigma_x, sigma_y, rho, MAX, theta_1_0, theta_2_0):
