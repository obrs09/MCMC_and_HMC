...
class roc(object):
        ...
        for j in range(0, self.thres.shape[0]):
            ...
            for i in range(0, self.prob_train.shape[0]):
                if self.prob_train[i][1] >= self.thres[j]:
                    ...
                    y_pred.append(0)
            y_pred = np.array(y_pred)
            self.tn_train, self.fp_train, self.fn_train, self.tp_train = confusion_matrix(self.y_true_train, y_pred, labels=[0, 1]).ravel()
            self.tnl_train.append(self.tn_train)
            self.fpl_train.append(self.fp_train)
            self.fnl_train.append(self.fn_train)
            self.tpl_train.append(self.tp_train)
            self.acc_train.append(round((self.tp_train + self.tn_train) / (self.tp_train + self.fp_train + self.fn_train + self.tn_train), 3))
    ...
    def crit_point_test(self):
        ...
        self.cd_test = np.where(distances == np.max(distances))
